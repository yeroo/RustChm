...
This document is the published code-signing policy for **rustchm**.
...
```
Get-FileHash .\rustchm-windows-x86_64.exe -Algorithm SHA256
# compare against rustchm-windows-x86_64.exe.sha256
```

Provenance (GitHub CLI):

```
gh attestation verify rustchm-windows-x86_64.exe --repo yeroo/RustChm
```
...
rustchm is a local command-line tool. It performs no network access and
